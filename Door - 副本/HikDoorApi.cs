...
namespace HikDeviceApi.Door
{
    ...
    public static class HikDoorApi
    {
        ...
        /// <returns>异步登录的状态、用户ID和设备信息通过NET_DVR_USER_LOGIN_INFO结构体中设置的回调函数(fLoginResultCallBack)返回。对于同步登录，接口返回-1表示登录失败，其他值表示返回的用户ID值</returns>
         [DllImport(@"HIKDLL\HCNetSDK.dll")]
        public static extern int NET_DVR_Login_V40(HikDeviceApi.Door.HikDoorStruct.NET_DVR_USER_LOGIN_INFO pLoginInfo, ref HikDeviceApi.Door.HikDoorStruct.NET_DVR_DEVICEINFO_V40 lpDeviceInfo);
        ...
        /// </remarks>
         [DllImport(@"HIKDLL\HCNetSDK.dll")]
        public static extern int NET_DVR_StartRemoteConfig(int lUserID, uint dwCommand, IntPtr lpInBuffer, uint dwInBufferLen, HikDeviceApi.Door.HikDoorDelegate.fRemoteConfigCallback cbStateCallback, IntPtr pUserData);
        ...
        [DllImport(@"HIKDLL\HCNetSDK.dll")]
        public static extern int NET_DVR_SetupAlarmChan_V41(int lUserID, ref HikDeviceApi.Door.HikDoorStruct.NET_DVR_SETUPALARM_PARAM lpSetupParam);
        /// <summary>
